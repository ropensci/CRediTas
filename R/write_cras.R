...
#' @param overwrite If TRUE, the file is overwritten. Otherwise, a error is triggered.
#' @param markdown If TRUE, the authors are surrounded by ** to make them bold in markdown.
...
#' cras_table[,2:ncol(cras_table)] <- sample(0:1, (ncol(cras_table)-1)*2, replace = TRUE)
#'
...
write_cras <- function(cras_table, file = "cras.txt", overwrite = FALSE, markdown = FALSE){
  cras <- character()
